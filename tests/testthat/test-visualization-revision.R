...
test_that("differential-expression data distinguishes unavailable adjusted P in both views", {
...
test_that("differential-expression table projects readable result columns", {
  ...
  expect_false(any(
    c("Mean Count Control", "Mean Count E-Stim") %in% names(observed)
  ))
})

test_that("MA and volcano plots omit unavailable adjusted P and retain each plotted gene once", {
  ...
  for (points in list(ma_points, volcano_points)) {
    expect_equal(nrow(points), sum(!is.na(primary_de$padj)))
    expect_identical(anyDuplicated(points$key), 0L)
    expect_setequal(points$key, primary_de$gene[!is.na(primary_de$padj)])
  }
  ma_built <- plotly::plotly_build(ma)
  expect_identical(
    ma_built$x$layout$legend$title$text,
    "FDR status · threshold 0.05"
  )
  ma_symbols <- unlist(
    lapply(ma_built$x$data, function(trace) {
  ...
  expect_identical(
    volcano_built$x$layout$legend$title$text,
    "FDR status · threshold 0.05"
  )
  volcano_symbols <- unlist(
    lapply(volcano_built$x$data, function(trace) {
      trace$marker$symbol %||% character()
    }),
    use.names = FALSE
  )
  expect_false("x" %in% volcano_symbols)
  ...
  for (view in c("ma", "volcano")) {
    ...
    full_point_layer <- Filter(
      function(layer) {
        nrow(layer) == sum(!is.na(primary_de$padj)) && "shape" %in% names(layer)
      },
      built$data
    )[[1L]]
...
test_that("tiny and zero adjusted P values remain plotted while missing values are omitted", {
  primary_de <- data.frame(
    gene = c("ZeroP", "TinyP", "MissingP"),
    baseMean = c(100, 200, 5),
    log2FoldChange = c(2, -2, 1),
    lfcSE = c(0.1, 0.2, 1),
    padj = c(0, 1e-300, NA_real_)
  )
  for (view in c("ma", "volcano")) {
    points <- visualization_plotly_points(make_de_plotly(
      primary_de,
      active_gene = "MissingP",
      source = "tiny_p_test",
      view = view
    ))
    expect_setequal(points$key, c("ZeroP", "TinyP"))
    expect_true(all(is.finite(points$y)))
    if (identical(view, "volcano")) {
      expect_equal(points$y[points$key == "TinyP"], 300)
      expect_equal(
        points$y[points$key == "ZeroP"],
        -log10(.Machine$double.xmin)
      )
    }
    static <- make_de_ggplot(primary_de, "MissingP", view)
    expect_setequal(static$data$gene, c("ZeroP", "TinyP"))
    expect_false(any(static$data$active))
  }
  displayed <- prepare_de_table_data(primary_de)
  expect_equal(nrow(displayed), 3L)
  expect_true(is.na(displayed$`Adjusted P`[displayed$Gene == "MissingP"]))
})
...
test_that("pathway plots keep GSEA and ORA on method-specific axes", {
  ...
  expect_true(grepl(
    ...
    observed$hover_text[[1L]],
    fixed = TRUE
  ))
  expect_true(grepl(
    "Overlapping genes",
    observed$hover_text[[2L]],
    fixed = TRUE
  ))
  ...
  keys <- unlist(
    lapply(interactive_built$x$data, function(trace) {
  ...
  expect_setequal(unique(static_built$data[[2L]]$shape), c(21, 23))
  ...
  expect_true(all(mapply(
    grepl,
    pattern = condition_label(observed_duplicate_label$direction),
...
test_that("pathway plots use the top ten results per method and direction", {
  pathways <- do.call(
    rbind,
    lapply(c("GSEA", "ORA"), function(source) {
      do.call(
        rbind,
        lapply(c("Control", "E-Stim"), function(direction) {
          index <- seq_len(15L)
          data.frame(
...
test_that("pathway plots add an active result outside the top ten", {
  ...
  interactive <- make_pathway_plotly(
    ...
    plotly::plotly_build()
  keys <- unlist(
    lapply(interactive$x$data, function(trace) {
      as.character(trace$key %||% character())
    }),
    use.names = FALSE
  )
  outline_widths <- unlist(
    lapply(interactive$x$data, function(trace) {
